...
#[rustler::nif]
fn generate_did(network_name: String) -> NifResult<(rustler::Atom, String)> {
    let result = runtime().block_on(async {
        generate_did_async(network_name).await
    });
...
async fn generate_did_async(network_name: String) -> Result<DidResult, String> {
    use identity_iota::iota::NetworkName;
    
    // Use a leaked string to get a static reference for NetworkName
    // This is acceptable for a NIF since network names are limited and reused
    let network_static: &'static str = Box::leak(network_name.clone().into_boxed_str());
...
#[rustler::nif]
fn extract_did_from_document(document_json: String) -> NifResult<(rustler::Atom, String)> {
...
#[rustler::nif]
fn create_did_url(did: String, fragment: String) -> String {
    format!("{}#{}", did, fragment)
}
...
#[rustler::nif]
fn is_valid_iota_did(did: String) -> bool {
...
// Register all NIF functions
rustler::init!("iota_did_nif");
